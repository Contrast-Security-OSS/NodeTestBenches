 ...
 */
    module.exports["crypto-bad-mac"] = async function exec(_inputs, {safe = false, noop = false} = {}) {
        if (noop)
            return "NOOP";

        const algorithm = safe ? "sha512" : "md5";

        return crypto.createHash(algorithm).update("salt").digest("hex");
    };
 ...
 * @param {boolean=} opts.noop are we calling the sink as a noop?
 */
    module.exports["crypto-bad-ciphers"] = async function exec(_inputs, {safe = false, noop = false} = {}) {
        if (noop)
            return "NOOP";
 ...
 * @param {boolean=} opts.noop are we calling the sink as a noop?
 */
    module.exports["crypto-weak-randomness"] = async function exec(_inputs, {safe = false, noop = false} = {}) {
        if (noop)
            return "NOOP";
